<template>
  ...
  <div
    class="hitsong-box"
    v-for="(item, i) in sonArray[index]"
    :key="item.name"
    @click="
      toPlay(
  ...
  >
    ...
    <div
      class="hit-mv"
      v-if="item.resourceExtInfo.song.videoInfo.video !== null"
    >
...
export default {
  ...
  data() {
    return {
      sonArray: [], // 通过监听获取父组件的数据
      saveArray: [],
      playingList: [], // 播放列表
      artistsList: [], // 歌手列表
      count: 0,
      hitTitle: 'hit-title', // 有文字content的title
      nullTitle: 'null-title', // 没有文字 content的title
    };
  },
  computed: {
    songListSave() {
